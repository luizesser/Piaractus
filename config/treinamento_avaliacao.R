tsne_plot <- function(df, df_bg, sp_names){
  ...
  result <- lapply(sp_names, function(sp){
    ...
    #df_bg_tsne <- df_pred %>%
    #  mutate(bio_8=case_when(prcts_brch==1 ~ 100, .default = bio_8)) %>%
    #  mutate(bio_5=case_when(prcts_brch==1 ~ 100, .default = bio_5)) %>%
    #  mutate(bio_3=case_when(prcts_brch==1 ~ 100, .default = bio_3)) %>%
    #  mutate(ord_stra=case_when(prcts_brch==1 ~ 100, .default = ord_stra)) %>%
    #  mutate(length_km=case_when(prcts_brch==1 ~ 100, .default = length_km)) %>%
    #  select(-sp) %>% 
      ...
      for (scenario_name in names(scenarios_list)){
        ...
        if (!dir_exists(folder_tmp)){
          ...
          df_pred_pres_aus <- df_pred_freq %>%
            ...
            mutate_all(funs(ifelse(. > 0.5, 1, 0))) %>% 
            mutate(consensus = rowMeans(.) %>% round(., digits = 0))
