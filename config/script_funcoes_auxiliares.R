...
DRE_predict <- function(df_p, m_treinados, algoritmo_predicao, tipo_thresh=2, lista_thresh=NULL){
  ...
  for (nome_coluna in colnames(predi)){
    if (is.na(as.integer(nome_coluna))){
      thresh <- lista_thresh %>% 
        filter(modelID==as.integer(stringr::str_split(nome_coluna, "__")[[1]][1]))
    } else {
      thresh <- lista_thresh %>% 
  ...
  for (i in 1:ncol(predi)){
    if (is.na(as.integer(colnames(predi[i])))){
      colnames(predi)[i] <- as.character(detalhes_modelos$method[detalhes_modelos$modelID==as.integer(stringr::str_split(colnames(predi[i]),"__")[[1]][1])])
    } else {
